...
public enum BrandImageProvider: Equatable {
    ...
    public static let logo: BrandImageProvider = .dynamicAsset({return UIImage.logo}, .scaleAspectFit)
    ...
    case dynamicAsset(() -> UIImage?, UIImageView.ContentMode)
    ...
    public static func == (lhs: BrandImageProvider, rhs: BrandImageProvider) -> Bool {
        switch(lhs, rhs){
        case(.dynamicAsset(let leftImg, let leftMode), .dynamicAsset(let rightImg, let rightMode)):
            return leftImg() == rightImg() && leftMode == rightMode
        case(.asset(let leftImg, let leftMode), .asset(let rightImg, let rightMode)):
            return leftImg == rightImg && leftMode == rightMode
        case (.dynamic(let leftLight, let leftDark), .dynamic(let rightLight, let rightDark)):
            return leftLight == rightLight && leftDark == rightDark
